...
def populate_attendance_data(ws, df: pd.DataFrame, dates: List[datetime], today: date):
    ...
    for i, dt in enumerate(dates):
        ...
        for j, (_, emp_row) in enumerate(df.iterrows()):
            ...
            checkins = sorted([l['time'] for l in logs if l['status'] == "Check-IN"])
            checkouts = sorted([l['time'] for l in logs if l['status'] == "Check-OUT"])
            ci_val, co_val = (min(checkins) if checkins else "-"), (max(checkouts) if checkouts else "-")
            
            ci_cell, co_cell = ws.cell(row_ci, col, ci_val), ws.cell(row_co, col, co_val)
            ci_ref, co_ref = ci_cell.coordinate, co_cell.coordinate
            
            minutes_formula = f'=IF(OR({ci_ref}="-",{co_ref}="-",{co_ref}<{ci_ref}),0,ROUND(({co_ref}-{ci_ref})*1440,2))'
            if is_sunday:
